...
with col_pipeline:
    ...
    def s(step):
        if not r:
            return "waiting"
        steps = ["search_results", "scrap_result", "report", "feedback"]
        if step in r:
            return "done"
    ...
    step_card("01", "Search Agent",  s("search_results"), "Gathers recent web information")
    step_card("02", "Reader Agent",  s("scrap_result"),    "Scrapes & extracts deep content")
    step_card("03", "Writer Chain",  s("report"),          "Drafts the full research report")
    step_card("04", "Critic Chain",  s("feedback"),        "Reviews & scores the report")
...
if r:
    ...
    if "scrap_result" in r:
        with st.expander("📄 Scraped Content (raw)", expanded=False):
            st.markdown(f'<div class="result-panel"><div class="result-panel-title">Reader Agent Output</div>'
                        f'<div class="result-content">{r["scrap_result"]}</div></div>', unsafe_allow_html=True)
